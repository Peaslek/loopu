import { numberWithCommas } from "../tasks/utils";
import {
  Coinmaster,
  Item,
  autosellPrice,
...
export function stats(): void {
  ...
  print("=== Account Stats === ");
  print("Misc", "orange");
  print(`Swagger ${get("availableSwagger")}`, "olive");
  print(`Karma ${get("bankedKarma")}`, "olive");
  print("Currencies:", "orange");
  items.forEach((i) => print(`${i} ${availableAmount(toItem(i))}`, "olive"));
  print();
}
...
function sum(record: Records, where: (key: string) => boolean): ProfitRecord {
  ...
  for (const key in record) {
    if (!(key.includes("Ignore") || key.includes("ignore"))) {
      if (where(key)) {
        included.push(record[key]);
      }
    }
  }
  return {
...
function printProfitSegment(
  ...
  if (combined) {
    print(
      ...
      )}: ${numberWithCommas(record.meat)} meat + ${numberWithCommas(
        record.items,
      )} items (${record.turns} turns + ${numberWithCommas(
        record.hours,
      )} hours)`,
      color,
    );
...
export function printProfits(records: Records): void {
  ...
  const sortedKeys = Object.entries(records)
    .sort((a, b) => {
      return -(a[1].meat + a[1].items - (b[1].meat + b[1].items));
    })
    .map(([key]) => key);
  ...
  for (const group of groups) {
    ...
    for (const key of groupKeys) {
      if (!(key.includes("ignore") || key.includes("Ignore"))) {
        printProfitSegment(`- ${key.split("@")[1]}`, records[key], "olive");
      }
    }
  }
